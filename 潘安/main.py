...
class CardPredictor:
    ...
    def predict(self, car_pic, model,transform_data):
        ...
        for rect in car_contours:
            ...
            if left_point[1] <= right_point[1]:  # 正角度
                ...
                # cv2.imshow("card", card_img)
                # cv2.waitKey(0)
        print("len(card_img)",len(card_img))
        # 开始使用颜色定位，排除不是车牌的矩形，目前只识别蓝、绿、黄车牌
        colors = []
        for card_index, card_img in enumerate(card_imgs):
            
            print("card_index", card_index, len(card_img))
            if(len(card_img) == 0):
                continue
        ...
        for i, color in enumerate(colors):
            ...
            if color in ("blue", "yello", "green"):
                ...
                for i, part_card in enumerate(part_cards):
                    ...
                    img = cv2.copyMakeBorder(part_card, 3, 3, 10, 10, cv2.BORDER_CONSTANT, value = [0,0,0])
                    ...
                    image = transform_data(PIL_image)
                    ...
                    # part_card = torch.from_numpy(part_card)
                    # with torch.no_grad:
                    #     out = model(part_card)
                    #     out = torch.max(out)
                    #     charactor = classes[out]
...
if __name__ == '__main__':
    ...
    r, roi, color = c.predict("./test/1.jpg", model,data_transform)
    print(r)
